...
var server = net.createServer(function (stream) {
  ...
  stream.addListener('connect', function () {
		...
		users.push(stream);
		user_games.push(game);
		stream.write("Welcome to game " + games.length + " -> ");
  });
  stream.addListener('data', function (data) {
		var game = user_games[users.indexOf(stream)];
		var parts = data.split(",");
		console.log(String(parts[0].trim()).length == String("setup").length);
		if( parts[0] == "setup" ) {
			var pos = [game.positions.x, game.positions.y, game.positions.rotation, game.positions.vx, game.positions.vy, game.positions.vr, game.positions.thrust];
			stream.write( "setup,"+pos.toString() );
			console.log("setup,"+pos.toString());
		}else if(parts[0]=="action"){
			for(i=0;i<game.users.length;i++){ 
				game.users[i].write( // tell other players that something what shot
					JSON.stringify({request:"action",type:input.type,rotation:input.rotation,x:input.x,y:input.y,vx:input.vx,vy:input.vy,player:users.indexOf(stream)})
				);
			}
			if(input.type == "move"){
				game.positions[users.indexOf(stream)] = {rotation:input.rotation,x:input.x,y:input.y,vx:input.vx,vy:input.vy}
			}
		}
		switch( parts[0] ){
			case "admin":
				if( login(input) ) {
					if( input.type="user" ) {
						socket.write(JSON.stringify({request:"info",data:users[input.user]}));
					}else if( input.type="game"){
